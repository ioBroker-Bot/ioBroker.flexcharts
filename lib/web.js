...
class ExtensionFlexcharts {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        this.rxDMauto = /autoDarkMode = false/; // Regular Expression to switch darkmode in echarts.html to update according to system setting
        this.strDMauto = 'autoDarkMode = true'; // Replacement for above RegEx
        ...
        this.applyDarkmode = function (content, queryDM) {
            if (queryDM == 'on') {
                ...
                content = new Buffer(content.toString().replace(this.rxDM, this.strDM));
            }
            if (queryDM == 'auto') {
                // Align dark mode to system setting
                content = new Buffer(content.toString().replace(this.rxDMauto, this.strDMauto));
            }
            return content;
        };
                                                    ...
                                                    if (typeof result == 'string') {
                                                        ...
                                                        if (Array.isArray(result)) {
                                                            ...
                                                            if (result.length > 1 && typeof result[1] == 'string') {
                                                                ...
                                                                content = new Buffer(
                                                                    content.toString().replace(this.rxOnEvent, r),
                                                                );
                                                            }
                                                        } else {
                                                ...
                                                if (typeof result.val == 'string') {
                                                    ...
                                                    if (Array.isArray(rp)) {
                                                        ...
                                                        if (rp.length > 1 && typeof rp[1] == 'string') {
                                                            content = new Buffer(
                                                                content.toString().replace(this.rxOnEvent, rp[1]),
                                                            );
                                                        }
                                                    } else {

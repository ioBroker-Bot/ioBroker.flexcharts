...
class ExtensionFlexcharts {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        this.rxTheme = /\/\* registerTheme\(\) \*\//; // Regular Expression to replace '/* registerTheme() */' in echarts.html
        this.rxThemePlain = '/* registerTheme() */'; // Plain version of replacement text in echarts.html
        this.rxOnEvent = /\/\* onEventFunctions\(\) \*\//; // Regular Expression to replace '/* onEventFunctions() */' in echarts.html
        this.rxOnEventPlain = '/* onEventFunctions() */'; // Plain version of replacement text in echarts.html
            ...
            if (typeof themeDef[0] == 'string' && typeof themeDef[1] == 'string') {
                ...
                if (r.length > 0) {
                    if (r.includes('registerTheme')) {
                        ...
                        content = new Buffer(content.toString().replace(this.rxTheme, r + this.rxThemePlain));
                    } else {
                        // JSON object of parameters of theme received
                        const f = `echarts.registerTheme("${themeName}",${r});`; // Function to register theme
                        content = new Buffer(content.toString().replace(this.rxTheme, f + this.rxThemePlain));
                    }
                }
            }
            return content;

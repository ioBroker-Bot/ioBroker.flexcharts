              ...
              if (query.source) {
                switch (query.source) {
                  case 'script':
                      ...
                      result => {
                        if (result.error) {
                          ...
                          if (typeof result == 'string') {
                            ...
                            content = new Buffer(content.toString().replace(this.rxChart ,result));
                          } else {
                            if (Array.isArray(result)) {
                              ...
                              if ( (result.length > 0) && (typeof result[0] == 'string') ) {
                                content = new Buffer(content.toString().replace(this.rxChart, result[0]));
                              }
                              if ( (result.length > 1) && (typeof result[1] == 'string') ) {
                                content = new Buffer(content.toString().replace(this.rxOnEvent, result[1]));
                              }
                            } else {
                    ...
                    break;
                  case 'state':
                    // Remark: It's not possible to transfer onEvent-Funktions via State.
                    //         This is due to implicit transformation of data of state to type String when using getForeignState()
                    if (query.id) {

...
class ExtensionFlexcharts {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
         ...
         */
        this.applyUserFunctions = function (content, userFct) {
        ...
        this.eCharts = async function (req, res) {
            ...
            var queryThemeV5 = false; // Query to use theme 'v5' as default theme. true => Use 'v5' theme
                                        ...
                                        result => {
                                            if (result.error) {
                                                ...
                                                if (result && result == 'Error: Timeout exceeded') {
                                                    ...
                                                    if (queryThemeV5) {
                                                        // Set Apache ECharts theme 'v5' as default
                                                        content = this.applyUserFunctions(content, this.strThemeV5);
                                                    }
                                                    if (typeof result == 'string') {
                                                        ...
                                                        if (Array.isArray(result)) {
                                                            ...
                                                            //   [>0] => if type array: Definition of default theme: [definition, name] (optional)
                                                            ...
                                                            while (i < result.length) {
                                                                if (typeof result[i] == 'string') {
                                                                    ...
                                                                    content = this.applyUserFunctions(
                                                                        content,
                                                                        result[i],
                                                                    );
                                                                } else {
                                                                    if (
                                                                        ...
                                                                        // Format of provides parameters is not supported
                                                                        this.adapter.log.error(
                                        ...
                                        this.adapter.getForeignState(query.id, (error, result) => {
                                            if (result && result.val) {
                                                ...
                                                if (typeof result.val == 'string') {
                                                    ...
                                                    if (Array.isArray(rp)) {
                                                        // Script contains an array => assumption: this is an array of strings containing additional elements:
                                                        //   [0] => String formatted chart options
                                                        //   [>0] => if type string: String formatted definition of dynamic function(s)
                                                        //   [>0] => if type array: Definition of default theme: [definition, name] (optional)
                                                        this.adapter.log.debug(
                                                        ...
                                                        while (i < rp.length) {
                                                            if (typeof rp[i] == 'string') {
                                                                // Process definition of user defined functions
                                                                content = this.applyUserFunctions(content, rp[i]);
                                                            } else {

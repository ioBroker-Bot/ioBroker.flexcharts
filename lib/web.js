...
class ExtensionFlexcharts {
    constructor(server, webSettings, adapter, instanceSettings, app) {
      ...
      this.eCharts = async function (req, res) {
        ...
        const replacementStr = '{ solution: 42 }';
        const replacementStrLen = replacementStr.length;
                      ...
                      result => {
                        ...
                        if (result.error) {
                          ...
                          this.demoChart(result => {
                            ...
                            content = new Buffer(content.toString().replace(replacementStr,JSON.stringify(result)));
                          ...
                          if (typeof result == 'string') {
                            ...
                            const posOptions = contentStr.search(replacementStr);
                            contentStr = contentStr.substring(0, posOptions) + 
                                         result +
                                         contentStr.substring(posOptions+replacementStrLen,contentStr.length);
                            content = new Buffer(contentStr);
                          } else {
                            // Script returned chart options as JSON object
                            // Unfortunately sendTo() has an underlaying JSON.stringify()-JSON.parse()-combination
                            // which filters function definitions
                            content = new Buffer(content.toString().replace(replacementStr,JSON.stringify(result)));
                          }
                          if ('darkmode' in query) content = new Buffer(content.toString().replace("document.getElementById('main'),null,","document.getElementById('main'),'dark',"));
                          res.end(content, 'utf-8');
                    ...
                    if (query.id) {
                      this.adapter.getForeignState(query.id, (error, result) => {
                        if ( (result) && (result.val) ) {
                          //this.adapter.log.debug(JSON.stringify(result.val));
                          res.writeHead(200, { 'Content-Type': contentType });
                          if ( (query.refresh) && (query.refresh >= 5) ) content = new Buffer(content.toString().replace('http-equiv="refresh" content="-3600"','http-equiv="refresh" content="'+String(query.refresh)+'"'));
                          content = new Buffer(content.toString().replace(replacementStr,String(result.val)));
                          if ('darkmode' in query) content = new Buffer(content.toString().replace("document.getElementById('main'),null,","document.getElementById('main'),'dark',"));
                          res.end(content, 'utf-8');
                        } else {
                          res.writeHead(200, { 'Content-Type': contentType });
